...
// #CRYPTO-DIRECT-1 — pannello del pagamento crypto diretto: l'utente scegli la
...
function CryptoDirectPanel({
  ...
  const [order, setOrder] = useState<{
    order: string; address: string; amount_coin: number; coin_label: string;
  } | null>(null);
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      try {
        const r = await fetch(`/api/crypto/status?order=${encodeURIComponent(order.order)}`);
  ...
  const start = async (coinId: string) => {
    setBusy(true); setError("");
    try {
      const r = await fetch("/api/crypto/checkout", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ requested_plan: plan, period, coin: coinId }),
      });
            ...
            {process.env.NEXT_PUBLIC_SHOPIFY_CRYPTO_ENABLED === "true" && (
              <>
                {!cryptoOpen ? (
                  ...
                  <CryptoDirectPanel plan={plan} period={period} onPaid={() => window.location.assign("/app?crypto=paid")} />
                )}
