                  ...
                  {(() => {
                    ...
                    return rows.flatMap((p, i) => {
                      const out: React.ReactNode[] = [
                        <PredictionCard key={p.match_id} p={p} idx={i} fp={quotaPartner(p.home_team, p.away_team, p.kickoff)} onSelect={onSelect} onBetNow={onBetNow} onGate={onGate} isPremium={isPremium} isFree={isFreeClient} autoOpen={autoOpenKey === `football:${p.match_id}`} saved={watchlist.saved.has(`football:${p.match_id}`)} onToggleWatch={watchlist.toggle} />,
                      ];
                      if (i === fpGridAt) {
                ...
                <div className="am-grid br-board-grid">
                  {(() => {
                    ...
                    return rows.flatMap((m, i) => {
                      const card = (
                        <TennisMatchCard key={m.id} m={m} idx={i} fp={fpOdds[teamPairKey("tennis", m.player1, m.player2, m.scheduled) ?? ""]} onSelect={onSelect} onBetNow={onBetNow} onGate={onGate} isPremium={isPremium} isFree={isFreeClient} autoOpen={autoOpenKey === `tennis:${m.id}`} saved={watchlist.saved.has(`tennis:${m.id}`)} onToggleWatch={watchlist.toggle} />
                      );
...
function CheckoutModal({
  ...
  // #CRYPTO-DIRECT-1: il pannello crypto si apre in linea, non naviga altrove.
...
function PredictionCard({ p, fp, onSelect, onBetNow, isPreview, isPremium, isFree, onGate, idx, autoOpen, saved, onToggleWatch }: { p: Prediction; fp?: FpOddsEntry; onSelect?: (s: SlipSelection) => void; onBetNow?: () => void; isPreview?: boolean; isPremium?: boolean; isFree?: boolean; onGate?: () => void; idx?: number; autoOpen?: boolean; saved?: boolean; onToggleWatch?: (key: string) => void }) {
  ...
  // Quota REALE di un mercato extra, o undefined. Mai derivata dalla probabilita'.
  const extraOdds = (key: string): number | undefined =>
    (e.extra_markets ?? []).find((m) => m.key === key)?.market_odds ?? undefined;
  ...
  const useHeadline = headline != null && headline.market !== "h2h";
  const headlineName = !useHeadline ? null
    : headline!.selection === "1X"
      ? pick5(lang, { it: `${p.home_team} o pareggio`, en: `${p.home_team} or draw`, es: `${p.home_team} o empate`, fr: `${p.home_team} ou nul`, ru: `${p.home_team} или ничья` })
      : pick5(lang, { it: `${p.away_team} o pareggio`, en: `${p.away_team} or draw`, es: `${p.away_team} o empate`, fr: `${p.away_team} ou nul`, ru: `${p.away_team} или ничья` });
  // Cio' che la testata mostra davvero. Il resto della card (schedina, mercati,
  // settlement) continua a lavorare su pickKey/pickProb: qui cambia solo la vetrina.
  const shownName = useHeadline ? headlineName : pickName;
  const shownProb = useHeadline ? headline!.prob : pickProb;
  const pickOdds =
  ...
  const mdsData: MdsData = (() => {
    ...
    return {
      ...
      hero: {
        ...
        // «vince»: non e' una scommessa che stiamo consigliando.
        pick: shownName
      ...
      labels: {
        ...
        placeBet: pick5(lang, { it: "Piazza la scommessa", en: "Place your bet", es: "Haz tu apuesta", fr: "Placez votre pari", ru: "Сделать ставку" }),
        ...
        side: pick5(lang, { it: "Schedina composta lato BetRedge → scegli il partner dove aprirla.", en: "Betslip composed on BetRedge → pick the partner to open it with.", es: "Boleto compuesto en BetRedge → elige el partner donde abrirlo.", fr: "Coupon composé sur BetRedge → choisissez le partenaire pour l'ouvrir.", ru: "Купон собран в BetRedge → выберите партнёра, чтобы открыть его." }),
...
export function TennisMatchCard({ m, fp, onSelect, onBetNow, isPreview, isPremium, isFree, onGate, idx, autoOpen, saved, onToggleWatch }: { m: TennisMatch; fp?: FpOddsEntry; onSelect?: (s: SlipSelection) => void; onBetNow?: () => void; isPreview?: boolean; isPremium?: boolean; isFree?: boolean; onGate?: () => void; idx?: number; autoOpen?: boolean; saved?: boolean; onToggleWatch?: (key: string) => void }) {
  const [showWhy, setShowWhy] = useState(false);
