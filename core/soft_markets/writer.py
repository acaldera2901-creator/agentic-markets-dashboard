import re
import unicodedata
from core.soft_markets.model import predict_lambda, p_over, MAIN_LINE, IS_GENERIC
from core.soft_markets.referee import referee_multiplier

MODEL_VERSION = "soft-leagueagnostic-v1"
_NOISE = re.compile(r"\b(FC|CF|SC|AC|AS|SV|SS|US|SSC|AFC|Calcio)\b", re.I)

def norm_name(s):
    # Fold diacritici (Göteborg->Goteborg) IDENTICO al normName TS di lib/odds-api.ts:
    # le fonti divergono sui segni e il join cross-source falliva. (#SOFT-MARKETS)
    s = unicodedata.normalize("NFKD", s or "")
    s = "".join(c for c in s if not unicodedata.combining(c))
    s = _NOISE.sub("", s)
    return re.sub(r"\s+", " ", s).strip().lower()

def build_rows(home, away, kickoff_iso, league, rates, referee=None):
    date = kickoff_iso[:10]
    key = f"{norm_name(home)}|{norm_name(away)}|{date}"
    rows = []
    for m, r in rates.items():
        lam = predict_lambda(m, r["a_h"], r["d_h"], r["a_a"], r["d_a"], r["glob"])
        # #REFEREE-BIAS-1: i cartellini dipendono dall'arbitro (backtest +~0.9% Brier).
        # Moltiplicatore fail-safe (arbitro sconosciuto → 1.0). Solo mercato "cards".
        if m == "cards" and referee:
            lam *= referee_multiplier(referee)
        line = MAIN_LINE[m]
        rows.append({
            "match_key": key, "league": league, "home_team": home, "away_team": away,
            "kickoff": kickoff_iso, "market": m, "expected": round(lam, 2),
            "main_line": line, "p_over": round(p_over(lam, line), 4),
            "confidence": None, "is_generic": IS_GENERIC[m], "model_version": MODEL_VERSION,
        })
    return rows
