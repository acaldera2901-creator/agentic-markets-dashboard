...
async def get_pitcher_fip(pitcher_id: Optional[int], season: int, lg_fip: dict) -> dict:
    """{fip, outs, games, starts} season-to-date, regressed toward league with
    K_OUTS fake outs.
    ...
    `games` e `starts` servono al flag opener (#NEWSPORTS, v2.1 dall'autopsia
    shadow del 14/07): un rilievo listato come partente probabile — molte
    presenze, quasi zero partenze — significa che il "duello fra i partenti" su
    cui poggia il why non esiste (caso reale: Wandy Peralta, 05/07). È un FLAG,
    non un blocco: la probabilità servita è del mercato e resta valida, ma il
    why non deve raccontare un duello che non c'è.
    """
    lg_val = 3 * lg_fip["corePerOut"] + lg_fip["const"]
    if not pitcher_id:
        return {"fip": lg_val, "outs": 0, "games": 0, "starts": 0}
    data = await _get_json(
