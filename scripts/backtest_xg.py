...
def run() -> None:
    ...
    print(f"{'+ xG form (attack/defence)':<40}{b_xg:>12.5f}")
    print(f"{'Market (Pinnacle, reference)':<40}{MARKET_BRIER:>12.5f}")
    print("=" * 60)
    gap = b_poi - MARKET_BRIER
    print(f"base closes {(b_poi - b_base) / gap * 100:+.1f}% of gap; "
          f"+xG closes {(b_poi - b_xg) / gap * 100:+.1f}%")
    print(f"xG marginal: {b_base:.5f} -> {b_xg:.5f}  ({(b_base - b_xg) / b_base * 100:+.2f}%)")
    coefs = dict(zip(["pH", "pD", "pA", "pi_diff", "form_diff", "xg_off", "xg_def"],
                     np.round(np.abs(clf_x.coef_).mean(axis=0), 3)))
