...
def test_dal_vecchio_indirizzo_si_arriva_al_settore_sala(in_piedi):
    """Dal 04/09 la sala non e' piu' una pagina a se': e' il settore C del
    piano unico. Il vecchio indirizzo non deve morire su un 404 — porta al
    settore, e il settore c'e' davvero nella pagina servita."""
    class SenzaSeguire(urllib.request.HTTPRedirectHandler):
...
def _js(*nomi: str) -> str:
    """Estrae dalla pagina, **per nome**, i pezzi di JS che servono al test.

    La pagina e' un HTML monolitico col JS dentro, e un test che cerca una
    stringa nell'HTML passa anche solo per un commento (verificato): il JS si
    esegue davvero sotto node. Estrarre per nome invece che per intervallo
    ("da questa funzione fino a quell'altra") tiene il test in piedi quando fra
    i due pezzi ne compare un terzo.
    """
    testo = PAGINA.read_text(encoding="utf-8")
    pezzi = []
    for n in nomi:
        if n.startswith("const "):          # una costante: la sua riga
            i = testo.index(n)
            pezzi.append(testo[i:testo.index("\n", i)])
        else:                               # una funzione: fino al `}` in colonna 0
            i = testo.index(f"function {n}(")
            pezzi.append(testo[i:].split("\n}", 1)[0] + "\n}")
    return "\n".join(pezzi)
...
@pytest.mark.skipif(shutil.which("node") is None, reason="serve node")
def test_nell_anello_la_delega_dice_a_chi_e_da_quando():
    """Si esegue davvero `deleghe()` sotto node, coi suoi tre pezzi estratti
    per nome (un test che cerca la stringa nell'HTML passa anche per un
    ...
    """ % _js("const COL", "breve", "const taglia", "deleghe")
    esito = subprocess.run(["node", "-e", prova], capture_output=True, text=True)
    assert esito.returncode == 0, esito.stderr
