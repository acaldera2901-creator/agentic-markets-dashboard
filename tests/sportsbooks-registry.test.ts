...
// parse-failure: JSON malformato -> parsePaths deve swallowre l'errore e restituire undefined
process.env.SPORTSBOOK_STAKE_PATHS = "{not valid json";
const booksInvalid = allSportsbooks();
assert.equal(booksInvalid.length, 1);
assert.equal(booksInvalid[0].sportPaths, undefined);
