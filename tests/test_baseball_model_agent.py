...
(the model never moves it); tiers per settings (floor v2.2 65/72);
warm-up and doubleheader rules from the Gate 1 report / lab audits.

Da #NEWSPORTS-V22-0801 coprono anche le TRE regole della v2.2 che mancavano
all'agente: cap serie, flag opener e finestra al quasi-close. Sono le regole nate
dall'autopsia del primo shadow (premium 3/9 causato da otto ripetizioni dello
stesso bet): se un giorno spariscono, questi test devono diventare rossi.
"""
import json
...
def test_assign_tier_boundaries_and_warmup():
    # #NEWSPORTS-V22-0801: floor 65/72 della v2.2, non i 62/65 del Gate 1 — il
    # loop premium ha misurato la banda 62-65 al 63,4% e il salto a 72.
    assert assign_tier(0.64, True, True) is None
    assert assign_tier(0.65, True, True) == "standard"
    assert assign_tier(0.71, True, True) == "standard"
    assert assign_tier(0.72, True, True) == "premium"
    # warm-up blocks everything
    assert assign_tier(0.80, True, False) is None


def test_assign_tier_disagreement_blocks_premium_not_standard():
    assert assign_tier(0.75, False, True) == "standard"
...
# ── #NEWSPORTS-V22-0801: le tre regole della v2.2 ────────────────────────────

NOW = datetime(2026, 8, 1, 18, 0, tzinfo=timezone.utc)


def _row(home, away, when):
    return {"home_team": home, "away_team": away, "published_at": when.isoformat()}


def test_pair_key_ignores_home_away_order_and_case():
    # In una serie MLB le squadre si alternano in casa: la chiave deve essere la
    # stessa, altrimenti il cap non vede il resto della serie.
    assert pair_key("Los Angeles Dodgers", "Colorado Rockies") == pair_key(
        "colorado rockies", "  Los Angeles Dodgers "
    )
    assert pair_key("A", "B") != pair_key("A", "C")


def test_series_cap_blocks_the_same_series_inside_the_window():
    recent = [_row("Los Angeles Dodgers", "Colorado Rockies", NOW - timedelta(days=2))]
    assert series_capped("Colorado Rockies", "Los Angeles Dodgers", NOW, recent, 5) is True


def test_series_cap_releases_after_the_window():
    recent = [_row("Los Angeles Dodgers", "Colorado Rockies", NOW - timedelta(days=6))]
    assert series_capped("Colorado Rockies", "Los Angeles Dodgers", NOW, recent, 5) is False


def test_series_cap_ignores_other_series():
    recent = [_row("New York Yankees", "Boston Red Sox", NOW - timedelta(hours=1))]
    assert series_capped("Colorado Rockies", "Los Angeles Dodgers", NOW, recent, 5) is False


def test_series_cap_is_fail_closed_on_an_unreadable_date():
    # Meglio una pick in meno che un campione correlato: data illeggibile o
    # assente = si considera dentro la finestra.
    for bad in ("non-una-data", None, ""):
        recent = [{"home_team": "Dodgers", "away_team": "Rockies", "published_at": bad}]
        assert series_capped("Rockies", "Dodgers", NOW, recent, 5) is True


def test_series_cap_disabled_when_zero_days():
    recent = [_row("Dodgers", "Rockies", NOW)]
    assert series_capped("Rockies", "Dodgers", NOW, recent, 0) is False


def test_series_cap_reproduces_the_dodgers_homestand():
    """Il caso reale che ha rotto il primo shadow: cinque partite della stessa
    serie in cinque giorni. La prima passa, le altre quattro no."""
    recent = []
    taken = 0
    for day in range(5):
        when = NOW + timedelta(days=day)
        if not series_capped("Los Angeles Dodgers", "Colorado Rockies", when, recent, 5):
            taken += 1
            recent.append(_row("Los Angeles Dodgers", "Colorado Rockies", when))
    assert taken == 1


def test_hours_to_start_and_unreadable_dates():
    assert hours_to_start("2026-08-01T22:00:00Z", NOW) == pytest.approx(4.0)
    assert hours_to_start("2026-08-01T12:00:00Z", NOW) == pytest.approx(-6.0)
    # Illeggibile o assente -> None, e il chiamante non pubblica.
    assert hours_to_start("domani sera", NOW) is None
    assert hours_to_start(None, NOW) is None


def test_hours_to_start_treats_naive_timestamps_as_utc():
    assert hours_to_start("2026-08-01T21:00:00", NOW) == pytest.approx(3.0)


def test_opener_flag_catches_a_reliever_listed_as_starter():
    # Il caso Wandy Peralta (05/07): 30 presenze, 1 partenza.
    flag = opener_flag("Wandy Peralta", {"games": 30, "starts": 1})
    assert flag is not None and "opener" in flag


def test_opener_flag_silent_on_a_real_starter():
    assert opener_flag("Tyler Glasnow", {"games": 20, "starts": 20}) is None
    # Pochi ingressi: non abbastanza per dire niente, quindi niente flag.
    assert opener_flag("Rookie X", {"games": 4, "starts": 0}) is None
    # Nessun nome = nessun flag (il why non nomina nessuno).
    assert opener_flag(None, {"games": 30, "starts": 0}) is None
