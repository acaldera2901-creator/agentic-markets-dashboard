...
- **Il `CookieBanner` vive dentro `app/app/page.tsx`, non nel root layout**: su `/`, `/tools`, `/weekly-pick` non compare. Conseguenza diretta: chi non passa dal desk non esprime mai un consenso, quindi qualsiasi cosa gated sul consenso è di fatto spenta lì (attribuzione compresa, e il signup della home passa da `HomeAuthModal`). Spostarlo è una decisione di prodotto, non un fix tecnico.
