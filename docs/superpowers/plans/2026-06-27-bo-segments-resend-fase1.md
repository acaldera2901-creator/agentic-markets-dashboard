...
- [ ] **⚠️ VERIFICA-PERNO upsert Resend (bloccante go-live):** confermare contro l'API reale che `POST /contacts` con `audience_id` nel body fa **upsert per email** (re-sync di un contatto esistente NON dà errore) e che l'array **`segments` SOSTITUISCE** l'appartenenza; confermare che `first_name` e `properties` (snake_case) siano accettati. Se l'upsert non è supportato così, adeguare `lib/resend-contacts.ts` (es. create-then-PATCH per email). Finché non confermato, NON dichiarare la feature operativa.
- [ ] **Sync verso audience di test:** premere "Sync su Resend" (= refresh completo dei segmenti attivi). Su Resend i contatti idonei compaiono con `properties` (`plan`, `language`, `lifecycle_stage`, `cohort_month`) e nell'array `segments` (es. `pro_it`). L'appartenenza è solo nell'array `segments` (niente proprietà `seg_*`).
- [ ] **Audit:** dopo un sync, in tabella `notifications` compare una riga `type='sync'` con il riepilogo `segments=.. contacts=.. ok=.. failed=..`.
