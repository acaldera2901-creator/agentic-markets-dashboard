...
export default function Countdown() {
  ...
  if (left <= 0) {
    return (
      <div className="wc-countdown wc-countdown-live">
        <span className="wc-live-dot" /> {wcT("tournamentLive", lang)}
      </div>
    );
  }
  const p = parts(left);
  const cells: Array<[number, "days" | "hrs" | "min" | "sec"]> = [
    [p.days, "days"],
    [p.hours, "hrs"],
    [p.minutes, "min"],
    [p.seconds, "sec"],
  ...
  return (
    <div className="wc-countdown" role="timer" aria-label="Countdown to World Cup kickoff">
      {cells.map(([v, id]) => (
        <span key={id} className="wc-countdown-cell">
          <strong>{String(v).padStart(2, "0")}</strong>
          <small>{wcT(id, lang)}</small>
        </span>
      ))}
    </div>
  );
