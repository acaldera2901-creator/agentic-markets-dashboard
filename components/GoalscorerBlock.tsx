...
export default function GoalscorerBlock({
  ...
  // #GOLIVE: mostriamo SOLO la nostra prediction. La colonna Mercato/Edge compare
  // unicamente se esiste davvero una quota: mai "–" né copy apologetico sui dati.
        ...
        <ul className="gs-list">
          {rows.map((m, i) => (
            <li key={m.playerId ?? `${m.side}-${m.name}-${i}`} className="gs-row">
              ...
              <span className="gs-model" title={p5(lang, { it: "La nostra probabilità che segni", en: "Our probability he scores", es: "Nuestra probabilidad de que marque", fr: "Notre probabilité qu'il marque", ru: "Наша вероятность гола" })}>{pct(m.pScores)}</span>
              {hasAnyOdds && <span className="gs-market">{m.marketImplied != null ? pct(m.marketImplied) : "–"}</span>}
              {hasAnyOdds && <span className={`gs-edge${m.edge != null && m.edge > 0 ? " pos" : ""}`}>{edgeTxt(m)}</span>}
            </li>
          ))}
        </ul>
      </div>
  ...
  return (
    <div className={`gs-block${hasAnyOdds ? "" : " gs-noodds"}`}>
      <div className="gs-head">
        ...
        <span className="gs-cols" aria-hidden="true">
          <span>{p5(lang, { it: "Nostra prob.", en: "Our prob.", es: "Nuestra prob.", fr: "Notre prob.", ru: "Наша вер." })}</span>
          {hasAnyOdds && <span>{p5(lang, { it: "Mercato", en: "Market", es: "Mercado", fr: "Marché", ru: "Рынок" })}</span>}
          {hasAnyOdds && <span>Edge</span>}
        </span>
      </div>
      ...
      <p className="gs-note">
        {hasAnyOdds
          ...
          : p5(lang, {
              it: "La nostra probabilità che ogni giocatore segni almeno un gol.",
              en: "Our probability that each player scores at least once.",
              es: "Nuestra probabilidad de que cada jugador marque al menos una vez.",
              fr: "Notre probabilité que chaque joueur marque au moins une fois.",
              ru: "Наша вероятность, что каждый игрок забьёт хотя бы раз.",
            })}
      </p>
    </div>
  );
